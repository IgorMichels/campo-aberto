...
model {
  ...
  phi_home ~ gamma(phi_prior_shape, phi_prior_rate);
  phi_away ~ gamma(phi_prior_shape, phi_prior_rate);
  ...
  for (n in 1:N) {
    ...
    target += game_weight[n]
      * (neg_binomial_2_lpmf(y_i[n] | mu, phi_home) + neg_binomial_2_lpmf(y_j[n] | lambda, phi_away));
  }
}
