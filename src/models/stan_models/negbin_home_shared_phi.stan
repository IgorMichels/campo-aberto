...
model {
  ...
  for (n in 1:N) {
    ...
    target += game_weight[n] * (neg_binomial_2_lpmf(y_i[n] | mu, phi) + neg_binomial_2_lpmf(y_j[n] | lambda, phi));
  }
}
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    log_lik += game_weight[n] * (neg_binomial_2_lpmf(y_i[n] | mu, phi) + neg_binomial_2_lpmf(y_j[n] | lambda, phi));
  }
}
