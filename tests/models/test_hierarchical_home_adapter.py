"""Unit tests for HierarchicalHomeAdapter.sample_scores/.sample_scores_single --
mirrors tests/models/test_poisson_home_no_rho_adapter.py: same math exactly
(the hierarchy only changes the Stan-side prior/fit structure, never how
scores are drawn from a posterior team_params dict -- see
src/models/adapters/hierarchical_home.py).
"""
...
from src.models.adapters.hierarchical_home import ADAPTER


def test_declares_its_own_name_and_parameter_shape():
    assert ADAPTER.name == "hierarchical_home"
    ...
    assert ADAPTER.stan_file.endswith("hierarchical_home.stan")
...
def test_sample_scores_home_and_away_goals_are_uncorrelated():
    """No Dixon-Coles-style correction here either -- home and away are drawn
    from independent Poissons, so their empirical covariance should sit near
    0."""
