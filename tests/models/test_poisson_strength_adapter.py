"""Unit tests for PoissonStrengthAdapter.sample_scores/.sample_scores_single --
mirrors tests/models/test_poisson_home_adapter.py, but with a single
"strength" team parameter standing in for both attack and defense (see
src/models/adapters/poisson_strength.py).
"""
...
from src.models.adapters.poisson_strength import ADAPTER


def test_declares_its_own_name_and_parameter_shape():
    assert ADAPTER.name == "poisson_strength"
    assert ADAPTER.team_param_names == ("strength",)
    assert ADAPTER.shared_param_names == ("eta", "beta_home", "rho")
    assert os.path.isfile(ADAPTER.stan_file)
    assert ADAPTER.stan_file.endswith("poisson_strength.stan")


def _random_team_params(n_draws, n_teams, rng):
    return {"strength": rng.normal(size=(n_draws, n_teams))}
...
def test_sample_scores_matches_the_poisson_means_when_rho_is_zero():
    ...
    should track mu_home/mu_away. Unlike poisson_home (separate attack/
    defense), here mu_home/mu_away both come from the same strength
    difference: mu_home = exp(strength[home] - strength[away] + eta +
    beta_home), mu_away = exp(strength[away] - strength[home] + eta)."""
    rng = np.random.default_rng(1)
    n_draws = 20000
    team_params = {"strength": np.full((n_draws, 2), [0.3, -0.1])}
    shared_params = {
    ...
    expected_mu_home = np.exp(0.3 - (-0.1) + 0.0 + 0.2)
    expected_mu_away = np.exp(-0.1 - 0.3 + 0.0)
...
def test_sample_scores_rho_shifts_the_scoreless_draw_frequency():
    ...
    n_draws = 20000
    team_params = {"strength": np.zeros((n_draws, 2))}
...
def test_sample_scores_and_sample_scores_single_agree_on_a_shared_fixture():
    ...
    team_params = {"strength": np.full((n_draws, n_teams), [0.4, -0.2, 0.1])}
    shared_params = {
        "eta": np.zeros(n_draws),
